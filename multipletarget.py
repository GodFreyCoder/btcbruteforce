...
def check_address(result):
    ...
    if compressed_address in target_addresses or p2pkh_uncompressed_address in target_addresses \
        ...
        with open('results.txt', 'a') as result_file:
            ...
            result_file.write(f"Compressed Address: {compressed_address}\n")
            result_file.write(
                f"P2PKH Uncompressed Address: {p2pkh_uncompressed_address}\n")
            result_file.write(
                f"P2SH Compressed Address: {p2sh_compressed_address}\n")
            result_file.write(
                f"Bech32 Compressed Address: {bech32_compressed_address}\n")
            result_file.write("-----------------------------------------------\n")
    else:
...
while not target_found:
    if sequential_mode:
        ...
        private_key = random.randint(start_int,
                                    end_int)  # Random search in random mode
